...
function UploadDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const upload = () => {
    ...
    if (follow) form.append("follow", "true")
    for (const file of files) form.append("file", file, file.name)
    const request = new XMLHttpRequest()
    request.open("POST", "/api/v1/uploads")
    request.upload.onprogress = (e) => e.lengthComputable && setProgress(e.loaded / e.total)
    request.onload = () => {
      setProgress(null)
      if (request.status >= 200 && request.status < 300) {
        ...
        try {
          setError((JSON.parse(request.responseText) as { message?: string }).message ?? "The upload didn't work.")
        } catch {
          setError(request.status === 413 ? "That's too big for this server to accept." : "The upload didn't work.")
        }
      }
    }
    request.onerror = () => {
