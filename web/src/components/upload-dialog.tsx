...
const usable = (file: File) =>
  AUDIO.test(file.name) || IMAGE.test(file.name) || ZIP.test(file.name);
...
function UploadDialog({
  ...
  const add = (more: File[]) => {
    ...
    setFiles((current) => {
      const seen = new Set(
        current.map((f) => `${f.webkitRelativePath || f.name}:${f.size}`),
      );
      return [
        ...current,
        ...keep.filter(
          (f) => !seen.has(`${f.webkitRelativePath || f.name}:${f.size}`),
        ),
      ];
    });
  ...
  const upload = () => {
    ...
    uploadFiles(
      ...
      .then(() => {
        ...
        toast(
          `Checking ${plural(tracks, "track")}; they'll be in Review shortly`,
        );
        reset();
        onClose();
      })
      .catch((e: unknown) => {
        ...
        setError(
          e instanceof UploadError
            ? e.message
            : "The upload didn't work. Check your connection and try again.",
        );
      })
      .finally(() => {
        ...
        <Dialog.Popup className="fixed inset-x-0 bottom-0 z-50 max-h-[92dvh] overflow-y-auto rounded-t-3xl border bg-card p-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] shadow-2xl outline-none sm:top-1/2 sm:bottom-auto sm:left-1/2 sm:w-[560px] sm:-translate-x-1/2 sm:-translate-y-1/2 sm:rounded-3xl sm:p-6">
          <div className="flex items-start gap-3">
            <div className="min-w-0 flex-1">
              <Dialog.Title className="type-title text-[22px]">
                Add music you have
              </Dialog.Title>
              <Dialog.Description className="mt-1 text-[14px] text-muted-foreground">
                Tracks, a folder or a zip. They're checked like any download,
                named from their tags, joined to the album if you already have
                part of it, and wait in Review.
              </Dialog.Description>
            </div>
            <Dialog.Close
              disabled={busy}
              ...
              <label className="flex items-center justify-between gap-3">
                <span>
                  <span className="block text-[14.5px]">
                    Keep this album complete
                  </span>
                  <span className="block text-[13px] text-muted-foreground">
                    Look for any tracks it's missing, now and as the artist adds
                    them.
                  </span>
                </span>
                <Switch
