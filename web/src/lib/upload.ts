      ...
      do {
        ...
        if (received !== undefined) {
          ...
          if (result.status === 200) {
            ...
            const perSecond =
              size / Math.max(0.2, (performance.now() - started) / 1000);
            piece = Math.max(
              MIN_PIECE,
              Math.min(
                session.max_chunk,
                Math.round(perSecond * PIECE_SECONDS),
              ),
            );
            failures = 0;
          }
        ...
        if ([400, 401, 403, 404, 413].includes(result.status)) {
          throw new UploadError(
            messageOf(result.body, "The upload was refused."),
          );
        }
        ...
        if (failures > RETRIES) {
          throw new UploadError(
            "The connection keeps dropping. Check it and try again.",
          );
        }
